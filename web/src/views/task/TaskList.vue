    ...
    >
      ...
      >
        ...
        <el-form-item label="负责人" prop="assignee_id">
          <el-select v-model="formData.assignee_id" style="width: 100%;">
            <el-option
        ...
        <el-form-item label="预计工时" prop="estimated_hours">
          <el-input-number v-model="formData.estimated_hours" :min="0" :step="0.5" style="width: 100%;" />
        </el-form-item>
    ...
    >
      <div class="task-detail" v-if="taskDetail">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="负责人">{{ taskDetail.assignee_name }}</el-descriptions-item>
          <el-descriptions-item label="创建人">{{ taskDetail.creator_name }}</el-descriptions-item>
          <el-descriptions-item label="截止日期">{{ taskDetail.deadline }}</el-descriptions-item>
          <el-descriptions-item label="预计工时">{{ taskDetail.estimated_hours || '-' }}h</el-descriptions-item>
          <el-descriptions-item label="实际工时">{{ taskDetail.actual_hours || 0 }}h</el-descriptions-item>
        </el-descriptions>
...
const handleAdd = () => {
  ...
  Object.assign(formData, {
    ...
    name: '',
    description: '',
    project_id: projectList.value[0]?.id || null,
    assignee_id: null,
    priority: 'medium',
    status: 'pending',
    deadline: '',
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        if (isEdit.value) {
          await updateTaskApi(formData.id, formData)
...
const handleConfirmReject = async () => {
  ...
  try {
    await rejectTaskApi(currentTaskId.value, rejectForm.reject_reason)
